...
export async function listPlayerForRent(
  app: FastifyInstance,
  userId: string,
  playerId: string,
  price: number,
  currency: "COIN" | "TON" = "COIN",
) {
  ...
  if (!teamPlayer) throw new Error("Player not found in your team");
...
export async function getAvailableRentals(app: FastifyInstance, userId: string) {
...
export async function rentPlayer(
  app: FastifyInstance,
  userId: string,
  playerId: string,
  days: number = 7,
  ...
  if (player.rent.isRented) throw new Error("Player already rented");
  ...
  if (isCoin) {
    if (user.coins < rentPrice) throw new Error("Not enough coins");
  } else {
    // TON logic...
  }
...
export async function checkExpiredRentals(app: FastifyInstance) {
    ...
    await app.prisma.$transaction([
      ...
      app.prisma.teamPlayer.deleteMany({
        where: {
          ...
          team: { userId: rent.rentedById as string },
        },
      }),
    ]);
  }
