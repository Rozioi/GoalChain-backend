...
export async function createPvPMatch(
  ...
  emitToUser(params.homeUserId, ServerEvent.MATCH_READY, { matchId: match.id });
  emitToUser(params.awayUserId, ServerEvent.MATCH_READY, { matchId: match.id });
...
export async function markPlayerReady(
  ...
  if (match.status !== "READY") throw new Error("Match is not in ready state");
...
function streamMatchEvents(
  ...
  seed: string,
  match: { homeUserId: string | null; awayUserId: string | null; isBot: boolean; homeTeamId: string; awayTeamId: string | null },
) {
...
async function finishLiveMatch(
  ...
  const rewards = await handleMatchCompletion(app, { ...match, id: matchId }, result, seed);
...
export async function updateLiveTactics(
  ...
  if (!match.awayTeamId || !match.seed) throw new Error("Match data incomplete");
  ...
  const lockedEvents = match.events
    ...
    .map((e) => ({
      minute: e.minute,
      type: e.type.toLowerCase() as "goal" | "yellowCard" | "redCard" | "save" | "chance" | "foul" | "injury" | "substitution",
      team: e.team,
      playerId: e.playerId,
      playerName: e.playerName,
      description: e.description,
