...
export function simulateMatch(
    ...
    const getTeamStrengths = (
        ...
        return {
            ...
            power:
                (attack * 0.6 + defense * 0.4 + rating * 0.1) * shortagePenalty,
        };
    };
    ...
    for (let minute = 1; minute <= totalMinutes; minute++) {
        ...
        for (const sub of currentSubs) {
            ...
            if (playerIdx !== -1 && benchIdx !== -1) {
                ...
                inPlayer.fatigue = 0;
                inPlayer.formValue = Math.min(
                    1.2,
                    (inPlayer.formValue || 1.0) + 0.1,
                );
        ...
        const currentTactics = pressingChanges.filter(
            (c) => c.minute === minute,
        );
        ...
        const lockedThisMinute = lockedEvents.filter(
            (e) => e.minute === minute,
        );
        if (lockedThisMinute.length > 0) {
            for (const e of lockedThisMinute) {
                ...
                const teamCards =
                    e.team === "home" ? homeYellowCards : awayYellowCards;
                ...
                if (e.type === "goal") {
                    ...
                    const idx = teamActive.findIndex(
                        (p) => p.id === e.playerId,
                    );
    ...
    return {
        ...
        homeStats: {
            possession: Math.round(
                (lastHomeS.power / (lastHomeS.power + lastAwayS.power) || 0.5) *
                    100,
            ),
            shots: homeShots,
        ...
        awayStats: {
            possession: Math.round(
                (lastAwayS.power / (lastHomeS.power + lastAwayS.power) || 0.5) *
                    100,
            ),
            shots: awayShots,
