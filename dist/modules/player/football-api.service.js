...
class FootballApiService {
    constructor(app) {
        this.app = app;
        this.apiKey = process.env.FOOTBALL_API_KEY || "9fcdea0c865d008bbaacd648b9bedd90";
        this.baseUrl = "https://v3.football.api-sports.io";
    }
    /**
     * Fetches players from API-Football for a specific league and page
     */
    async fetchPlayersFromApi(leagueId, season, page = 1) {
     ...
     */
    async importPlayersForLeague(leagueId, season = 2024) {
        ...
        do {
            ...
            const playersToCreate = playersData.map((item) => {
                const p = item.player;
                const s = item.statistics[0]; // Take first statistics entry
                // Simple position mapping
                let position = "ST";
                if (s.games.position === "Goalkeeper")
                    position = "GK";
                else if (s.games.position === "Defender")
                    position = "CB";
                else if (s.games.position === "Midfielder")
                    position = "CM";
                else if (s.games.position === "Attacker")
                    position = "ST";
                ...
                if (position === "GK")
                    ...
                    role = "DEFENDER";
                else if (["CM", "CDM", "CAM"].includes(position))
                    role = "MIDFIELDER";
     ...
     */
    async populateInitialDatabase(count = 68000) {
